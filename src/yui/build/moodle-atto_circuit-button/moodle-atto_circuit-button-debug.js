YUI.add('moodle-atto_circuit-button', function (Y, NAME) {
    ...
    var COMPONENTNAME = 'atto_circuit',
        ATTRSTOREINREPO = 'storeinrepo',
        REGEX = {
            ISPERCENT: /\d+%/
        },
        IFSOURCE = M.cfg.wwwroot + '/lib/editor/atto/plugins/circuit/circuit.html',
        IFID = 'circuitpad',
        SUBMITID = 'submit',
        CSS = {
        ...
        Y.namespace('M.atto_circuit').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
            /**
            ...
            _doInsert: function(e) {
                ...
                host.saveSelection();
                e = e._event;
                var canvas = document.getElementById(IFID).contentWindow.document.getElementById("canvas");
								
                //var circuitimage = canvas.toDataURL();
                //var circuitimage = self._convertImage(myLC.getImage().toDataURL());
                var circuitimage = self._convertImage(canvas.toDataURL());
                console.log(circuitimage);
                // Only handle the event if an image file was dropped in.
                var handlesDataTransfer = (circuitimage && circuitimage.size && circuitimage.type == 'image/png');
                console.log(handlesDataTransfer);
                if (handlesDataTransfer) {
                    ...
                    e.preventDefault();
                    e.stopPropagation();
                    formData.append('repo_upload_file', circuitimage);
            ...
            _displayDialogue: function(e, clickedicon) {
                ...
                dialogue = this.getDialogue({
                    headerContent: M.util.get_string('circuittitle', COMPONENTNAME),
                    width: width,
                    height: height,
            ...
            _doInsertBase64: function(e) {
			  ...
			  //var image = canvas.toDataURL();
                ...
                parent.editor.focus();
                parent.get('host').insertContentAtFocusPoint(circuit);
                parent.markUpdated();
            }
        }, {
        ATTRS: {
            /**
             * How to save circuites.
             *
